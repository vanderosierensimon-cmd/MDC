...
export default function Footer() {
  ...
  return (
    <footer className="bg-canal-navy text-white pb-20 lg:pb-0" role="contentinfo">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 text-sm text-center">
          ...
          <address className="not-italic text-white/70 space-y-1">
            <p className="font-semibold text-white mb-2">La Maison du Canal</p>
            <p>Rue du Canal 6, 7730 Leers-Nord</p>
            <a href="tel:+3256914285" className="hover:text-canal-gold transition-colors block">056 91 42 85</a>
            <p className="pt-1">Ouvert tous les jours · 11h – 21h</p>
          </address>
          ...
          <nav aria-label="Navigation du pied de page">
            ...
            <ul className="space-y-1 text-white/70">
              {[
                ...
                <li key={link.name}>
                  {link.external ? (
                    <a href={link.href} target="_blank" rel="noopener noreferrer" className="hover:text-canal-gold transition-colors">
                      {link.name}
                    </a>
                  ) : (
                    <Link href={link.href} className="hover:text-canal-gold transition-colors">
                      {link.name}
                    </Link>
                  )}
                </li>
          ...
          <div>
            ...
            <div className="flex gap-3 justify-center">
              <a
                ...
                aria-label="Facebook"
                className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center hover:bg-canal-gold transition-colors"
              >
